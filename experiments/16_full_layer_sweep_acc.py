"""
Full Layer Sweep - Accuracy-Based Classification

Same as experiment 15 but uses ACCURACY-ONLY classification instead of
entropy+accuracy. This gives a simpler, more interpretable categorization:

  - NOISE:    accuracy goes UP when chunk removed (even +1 question = noise)
  - SIGNAL:   accuracy goes DOWN when chunk removed
  - INERT:    accuracy unchanged (zero change)
  - CRITICAL: model produces NaN logits (catastrophic failure)

This runs alongside experiment 15 (entropy-based) to compare approaches.

Usage:
    # Paired MLP sweep (recommended — same as exp 14 style)
    python experiments/16_full_layer_sweep_acc.py --layers 0-31 --matrices mlp --paired

    # Separate matrix sweep
    python experiments/16_full_layer_sweep_acc.py --layers 0-31 --matrices mlp

    # Quick test
    python experiments/16_full_layer_sweep_acc.py --layers 31 --matrices mlp --paired --test
"""
...
def classify_chunk_by_accuracy(acc_change, entropy_is_nan):
    """
    Classify a chunk based on accuracy change only.

    Returns:
        label (str): 'noise', 'signal', 'inert', or 'critical'
    """
    if entropy_is_nan:
        return 'critical'
    elif acc_change > 0:
...
# =============================================================================
# Single layer+matrix experiment (accuracy-based)
# =============================================================================
...
def run_single_layer_matrix(
    ...
    Classification uses ACCURACY ONLY (not entropy).
    """
    result_path = get_result_path(output_dir, model_short, layer_idx, matrix_type)

    print(f"\n{'='*60}")
    print(f"Layer {layer_idx} | {matrix_type} (ACC-BASED)")
    print(f"{'='*60}")
    ...
    for chunk_idx in tqdm(range(num_chunks), desc=f"L{layer_idx}/{matrix_type}", leave=False):
        ...
        label = classify_chunk_by_accuracy(acc_change, np.isnan(mcq_entropy))
        ...
        if np.isnan(mcq_entropy_change):
            print(f"    ch{chunk_idx:>2} [{chunk_start:>4}:{chunk_end:>4}] "
                  f"ent=NaN  acc={acc_change*100:+.1f}pp  energy={energy_removed*100:.1f}%  → {label.upper()}")
        else:
            print(f"    ch{chunk_idx:>2} [{chunk_start:>4}:{chunk_end:>4}] "
                  f"ent={mcq_entropy_change:+.4f}  acc={acc_change*100:+.1f}pp  "
                  f"energy={energy_removed*100:.1f}%  → {label.upper()}")

        # Restore
        restore_original_weight(model, layer_idx, original_weight, matrix_type, model_type)

    # Classify chunks
    noise_chunks = [c['chunk_idx'] for c in chunk_results if c['classification'] == 'noise']
    signal_chunks = [c['chunk_idx'] for c in chunk_results if c['classification'] == 'signal']
    inert_chunks = [c['chunk_idx'] for c in chunk_results if c['classification'] == 'inert']
    critical_chunks = [c['chunk_idx'] for c in chunk_results if c['classification'] == 'critical']

    crit_str = f", {len(critical_chunks)} critical" if critical_chunks else ""
    print(f"  Classification: {len(noise_chunks)} noise, {len(signal_chunks)} signal, "
          f"{len(inert_chunks)} inert{crit_str}")

    # Cumulative noise removal
    cumulative_results = []
    if noise_chunks:
        noise_sorted = sorted(noise_chunks)
        chunk_range_map = {c['chunk_idx']: (c['chunk_start'], c['chunk_end']) for c in chunk_results}
        stacked_ranges = []

        print(f"\n  Cumulative noise removal ({len(noise_sorted)} noise chunks to stack):")
        print(f"  Baseline accuracy: {baseline_mcq_accuracy*100:.1f}%")
        ...
        for i, ci in enumerate(noise_sorted):
            ...
            chunks_str = ",".join(str(c) for c in noise_sorted[:i+1])
            acc_delta = (cum_accuracy - baseline_mcq_accuracy) * 100
            print(f"    cumul {i+1}/{len(noise_sorted)}: "
                  f"removed chunks [{chunks_str}] → "
                  f"acc={cum_accuracy*100:.1f}% ({acc_delta:+.1f}pp)  "
                  f"energy={energy_removed*100:.1f}%")

            restore_original_weight(model, layer_idx, original_weight, matrix_type, model_type)
    else:
        print(f"\n  No noise chunks found — skipping cumulative removal.")

    # Summary stats
    valid_chunks = [c for c in chunk_results if c['classification'] != 'critical']
    sum_entropy_change = sum(c['mcq_entropy_change'] for c in valid_chunks if not np.isnan(c['mcq_entropy_change']))
    sum_accuracy_change = sum(c['mcq_accuracy_change'] for c in valid_chunks)
    noise_fraction = len(noise_chunks) / len(chunk_results) if chunk_results else 0
    signal_fraction = (len(signal_chunks) + len(critical_chunks)) / len(chunk_results) if chunk_results else 0
    ...
    result = {
        'config': {
            ...
            'classification_method': 'accuracy_only',
        },
        'svd_stats': stats,
        'baseline_mcq': {
        ...
        'summary': {
            ...
            'num_noise': len(noise_chunks),
            'num_signal': len(signal_chunks),
            'num_inert': len(inert_chunks),
            'num_critical': len(critical_chunks),
        },
    }
    ...
    print(f"  Net accuracy: {sum_accuracy_change*100:+.1f}pp, "
          f"Noise fraction: {noise_fraction:.1%}, Signal fraction: {signal_fraction:.1%}")

    return result


# =============================================================================
# Paired matrix removal (accuracy-based)
# =============================================================================
...
def run_single_layer_paired(
    ...
    for chunk_idx in tqdm(range(num_chunks), desc=f"L{layer_idx}/{matrix_str}", leave=False):
        ...
        if np.isnan(mcq_entropy_change):
            print(f"    ch{chunk_idx:>2} [{chunk_start:>4}:{chunk_end:>4}] "
                  f"ent=NaN  acc={acc_change*100:+.1f}pp  energy={avg_energy_removed*100:.1f}%  → {label.upper()}")
        else:
            print(f"    ch{chunk_idx:>2} [{chunk_start:>4}:{chunk_end:>4}] "
                  f"ent={mcq_entropy_change:+.4f}  acc={acc_change*100:+.1f}pp  "
                  f"energy={avg_energy_removed*100:.1f}%  → {label.upper()}")
    ...
    if noise_chunks:
        ...
        print(f"\n  Cumulative noise removal ({len(noise_sorted)} noise chunks to stack, paired {matrix_str}):")
        print(f"  Baseline accuracy: {baseline_mcq_accuracy*100:.1f}%")
...
def generate_summary_csv(output_dir: Path, model_short: str):
    ...
    for pkl_file in sorted(output_dir.glob(f"{model_short}_layer*.pkl")):
        ...
        rows.append({
            ...
            'num_signal': summary['num_signal'],
            'num_inert': summary['num_inert'],
            'num_critical': summary['num_critical'],
...
def run_full_sweep(
    ...
    Uses accuracy-only classification.
    """
    seed_everything(42)
    ...
    output_dir = Path(f"results/full_sweep/{model_short}_acc_chunk{chunk_size}")
    ...
    print("=" * 70)
    print("FULL LAYER SWEEP (ACC-BASED) - Spectral Anatomy of Factual Noise")
    print("=" * 70)
    print(f"Model: {model_name}")
    print(f"Layers: {layers}")
    print(f"Matrices: {matrix_types}")
    print(f"Chunk size: {chunk_size}")
    print(f"Classification: ACCURACY-ONLY (noise/signal/inert/critical)")
    ...
    # Generate summary
    print("\n" + "=" * 70)
    print("SWEEP COMPLETE (ACC-BASED)")
    print("=" * 70)
    generate_summary_csv(output_dir, model_short)
...
def run_full_sweep_paired(
    ...
    matrix_str = "+".join(matrix_types)
    model_short = model_name.split("/")[-1]
    output_dir = Path(f"results/full_sweep/{model_short}_acc_paired_{matrix_str}_chunk{chunk_size}")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    total_jobs = len(jobs) + skipped
    print("=" * 70)
    print("FULL LAYER SWEEP (PAIRED, ACC-BASED) - Spectral Anatomy of Factual Noise")
    print("=" * 70)
    print(f"Model: {model_name}")
    print(f"Layers: {layers}")
    ...
    # Generate summary
    print("\n" + "=" * 70)
    print("PAIRED SWEEP COMPLETE (ACC-BASED)")
    print("=" * 70)
    generate_summary_csv(output_dir, model_short)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Full Layer Sweep (ACC-BASED) - Spectral Anatomy of Factual Noise"
    )
